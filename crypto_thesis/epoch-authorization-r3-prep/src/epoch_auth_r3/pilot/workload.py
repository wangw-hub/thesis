...
class R3PilotWorkloadGeneratorV1:
    DOMAIN = b"EPOCH_AUTH_R3_I9_PILOT_WORKLOAD_V1"
    ...
    def generate(cls, seed: int, size: int) -> bytes:
        ...
        while len(out) < size:
            out.extend(hmac.new(key, cls.DOMAIN + counter.to_bytes(8, "big"), hashlib.sha256).digest())
            counter += 1
