...
**引理4.2（层次覆盖的完整性与互斥性）。** 对任意规范区间 \(I=[a,b)\)，`DYADIC-COVER` 输出节点均位于 \(I\) 内、两两互斥，且节点并集等于 \(I\)。
...
**引理4.3（规范序列的唯一性）。** 固定 \(t_0,\Delta,U\) 与 NTP1 schema。若 \(S(P_1)=S(P_2)\)，则 \(I_1^*=I_2^*\) 且 \(B(P_1)=B(P_2)\)。


**证明。** 有限整数集合的极大连续分量分解唯一。由引理4.1，两个规范化结果均为同一集合的极大连续分量，故 \(I_1^*=I_2^*\)。NTP1对相同字段采用固定顺序、宽度和字节序，因而产生相同字节串。证毕。□


**引理4.4（最大层次覆盖的规范性）。** 固定二进制层次后，任意规范区间 \(I\) 的最大层次节点集合唯一，算法4-2输出该集合。


**证明。** 对 \(I\) 中任一槽，沿叶到根路径存在唯一最高节点仍完全包含于 \(I\)。所有这样的最高节点互不包含；结合引理4.2，它们构成完整互斥覆盖。算法4-2在每个未覆盖左端点选择最大可用对齐块，若存在更大合法父节点，则当前块不是最大，和选择规则矛盾。使用更小子节点又不满足最大性，故输出集合唯一。证毕。□


**定理4.1（PolicyCompile正确性）。** 对任意合法输入，算法4-1终止，并满足：
...
**证明。** 规范化扫描有限；覆盖循环中左端点严格增加；序列化与摘要处理有限长度输入，因此算法终止。第一项由引理4.1和引理4.2得到。\(I^*\) 与 \(B(P)\) 的唯一性由引理4.3得到，\(C(P)\) 的唯一性由引理4.4得到；确定性哈希函数对相同字节输入输出相同摘要。证毕。□
...
设 \(n\) 为原始区间数，\(k=|I^*|\)，\(c=|C(P)|\)。各阶段复杂度见本节分阶段分析。
...
E2作为E1性能实验的准入条件。性质测试沿用“将性质编码并在生成输入上自动检查”的方法思想[4]，但不以随机测试替代理论证明。正式实验前后以及补充实验前均执行全量测试，结果见表4-5。
...
**表4-6 E1-C核心边界结果（由 `e1_representation_ratios.csv` 汇总；各随机种子结构结果一致时取代表值）**
...
表5-2给出运行级总体统计。四种方法的端到端中位时延均约为 196--199 ms，吞吐量中位数约为 17.78--17.93 请求/s。
...
表5-3给出四种自然配对比较。点估计均很小，均值差的 95% Bootstrap 置信区间全部跨越 0；在预注册的 1 ms 工程阈值下，改善与退化运行比例相近。
...
表5-3 四种配对比较及运行级 Bootstrap 置信区间。正值表示前者较慢。
...
![图5-6 各阶段耗时构成](research-content-2-final/figures/figure-5-7-stage-share.png)


图5-6 `chain_read`、`match`、`issue` 和 `verify` 的运行级耗时构成。


正式运行期间链高度持续增长；冻结健康快照中的 `peerCount` 为 4。图5-7右图反映准入与结束健康快照，而非逐请求连续 peer 遥测。


![图5-7 正式运行期间链状态稳定性](research-content-2-final/figures/figure-5-8-chain-stability.png)


图5-7 正式运行期间区块高度及冻结健康快照。
...
![图5-8 局部性对缓存命中率的影响](research-content-2-final/figures/figure-5-6-locality-cache.png)
