# -*- coding: utf-8 -*-
"""M4: build the refined midterm DOCX from the user's original cover authority."""
...
SRC = ROOT / "docs/midterm-report/m4/M4-MIDTERM-SOURCE.md"
OUT = ROOT / "docs/midterm-report/m4/output"
OUT_DOCX = OUT / "王威-专业学位研究生学位论文中期考评表-M4候选稿.docx"
FIGDIR = ROOT / "docs/midterm-report/m4/figures"
...
FIGURE_MAP = [
    ("图1 论文总体技术路线与三项研究内容递进关系", FIGDIR / "m4-fig1-route.png"),
    ("图2 语义主表示—摘要—派生执行IR关系", FIGDIR / "m4-fig2-ir.png"),
    ("图3 非连续时间策略确定性编译流程", FIGDIR / "m4-fig3-compile.png"),
    ("图4 匹配查询中位时延（E1-A 正式实验结果）", FIGDIR / "m4-exp-fig8-match.png"),
    ("图5 三种表示的逻辑规模比较（E1-A 正式实验结果）", FIGDIR / "m4-exp-fig9-rep-size.png"),
    ("图6 表示的压缩比与适用边界（E1-A 正式实验结果）", FIGDIR / "m4-exp-fig10-boundary.png"),
    ("图7 许可联盟链可信授权系统总体架构", FIGDIR / "m4-fig4-arch.png"),
    ("图8 CAP2 签发与验证双泳道流程", FIGDIR / "m4-fig5-cap2.png"),
    ("图9 并发度对端到端时延的影响（RC2）", FIGDIR / "m4-exp-fig14-concurrency.png"),
    ("图10 四种方法的运行级端到端时延分布（RC2 正式实验结果）", FIGDIR / "m4-exp-fig12-latency.png"),
    ("图11 请求局部性与缓存的影响（RC2）", FIGDIR / "m4-exp-fig16-locality.png"),
    ("图12 端到端时延的阶段占比（RC2 中位数）", FIGDIR / "m4-exp-fig11-stage.png"),
    ("图13 自然配对比较与运行级 Bootstrap 置信区间（RC2）", FIGDIR / "m4-exp-fig13-paired.png"),
    ("图14 碎片率对匹配时延的影响（RC2）", FIGDIR / "m4-exp-fig15-frag.png"),
    ("图15 版本化密文对象结构（Header/Body/CK）", FIGDIR / "m4-fig6-object.png"),
    ("图16 链上可信状态—控制协调—链下密文对象三层闭环架构", FIGDIR / "m4-fig7-closure.png"),
    ("图17 HEADER_ONLY 操作端到端时延（RC3 正式实验结果）", FIGDIR / "m4-exp-fig17-header.png"),
    ("图18 BODY_ROTATION 操作端到端时延（RC3 正式实验结果）", FIGDIR / "m4-exp-fig18-rotation.png"),
    ("图19 LOCAL_ONLY 与 KUBO_REPLICA 恢复时延对比（RC3 E5）", FIGDIR / "m4-exp-fig19-recovery.png"),
]
...
def add_table(cell, header, rows, caption=None, font=8.5, align_left_cols=None):
    ...
    # three-line borders (top/bottom heavy, header underline)
...
def add_figure(cell, path, caption, width_cm=12.5):
    ...
    run = p.add_run()
    try:
        run.add_picture(str(path), width=Cm_SAFE(width_cm))
