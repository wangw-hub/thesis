# -*- coding: utf-8 -*-
"""FINAL-CLEAN: build the final frozen midterm DOCX from the FINAL source.

Same hardening as build_m7_docx.py (OMML empty-nary guard, repeat header rows,
cantSplit, cover single-check) plus:
  * corrected table 7 (8 rows, 4 fault classes x 2 source modes) with frozen
    E5 medians;
  * figure 20 caption aligned with the regenerated "comparable scenarios" plot.
"""
...
SRC = ROOT / "docs/midterm-report/final/FINAL-MIDTERM-SOURCE.md"
OUT = ROOT / "docs/midterm-report/final/output"
OUT_DOCX = OUT / "王威-专业学位研究生学位论文中期考评表-最终固化版.docx"
SYSTEM_FIG = Path(r"D:\Users\wangw\Desktop\中期和小论文\系统结构图")
EXP_FIG = ROOT / "docs/midterm-report/final/figures"
...
def latex_to_omml(latex: str) -> etree._Element:
    ...
    if om is None:
        om = root
    result = copy.deepcopy(om)
    # Fatal placeholder guard: an empty nary base (`<m:e/>`) renders as a
    # dashed box in Word. Every nary must have a non-empty argument.
    xml = etree.tostring(result, encoding="unicode")
    if re.search(r"<m:e/>", xml):
        raise SystemExit(
            "FORMULA_PLACEHOLDER_FOUND: empty <m:e/> nary base in OMML for:\n"
            + latex
            + "\nUse \\left...\\right around the big-operator argument."
        )
    return result
...
FIGURE_MAP = [
    ...
    ("图20 可比较故障场景下的恢复端到端时延（对象来源×故障场景，版本化密文生命周期实验）", EXP_FIG / "m6-exp-fig20-e5-recovery.png", 12.8),
]
...
TABLE_DATA = {
    ...
    "故障恢复实验结果与时长汇总（按故障与对象来源；内容标识不一致与双端缺失场景按 Fail-Closed 终止，不纳入恢复时延比较）": (
...
def add_table(cell, header, rows, caption=None, font=8.5, align_left_cols=None):
    ...
    # --- M7 table-layout hardening -------------------------------------
    # header row repeats on every page when the table must split
    tr0 = t.rows[0]._tr
    trPr0 = tr0.get_or_add_trPr()
    tblHeader = OxmlElement("w:tblHeader")
    trPr0.append(tblHeader)
    # rows never split mid-row; paragraphs chain so a table that fits on one
    # page moves as a whole instead of breaking with an orphan header
...
def main():
        ...
        if "攻读学位级别" in t:
            for r in para.runs:
                if r.text == "□":
                    ...
                    r.text = "☑硕士"
            # M7: remove the duplicate Wingdings 2 F052 checkbox sym so only
            # the single "☑硕士" check remains on the degree-level line.
            for r in list(para.runs):
                if r._element.find(qn("w:sym")) is not None:
    ...
    t0 = doc.tables[0]
    # M7: keep template form labels with their following content so section
    # headings never dangle at the bottom of a page.
    label_keys = ("论文研究进展", "阶段性研究成果", "未按开题计划", "针对上述问题采取何种解决办法", "中期考评审查意见")
    for row in t0.rows:
        for cell in row.cells:
            for p in cell.paragraphs:
                if any(k in p.text for k in label_keys):
    ...
    doc.save(OUT_DOCX)
    # post-build placeholder audit on the saved DOCX
    saved = Document(str(OUT_DOCX))
    bad = 0
    for om in saved.element.body.iter(qn("m:oMath")):
        xml = etree.tostring(om, encoding="unicode")
        if "<m:e/>" in xml:
            bad += 1
    if bad:
        raise SystemExit(f"POST_BUILD_OMML_EMPTY_NARY={bad}")
