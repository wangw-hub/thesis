# -*- coding: utf-8 -*-
"""M7: build the final-candidate midterm DOCX from the M7 source.

Differences vs build_m6_docx.py:
  * OMML empty-nary guard: formulas must not contain an empty nary base
    (`<m:e/>`), which Word renders as a dashed placeholder box.
  * tables: header row repeats on page breaks (`w:tblHeader`), rows do not
    split (`w:cantSplit`), and the whole table is kept together where it fits.
  * cover: removes the duplicate Wingdings checkbox sym on the degree-level
    line so only one check mark remains.
"""
...
SRC = ROOT / "docs/midterm-report/m7/M7-MIDTERM-SOURCE.md"
OUT = ROOT / "docs/midterm-report/m7/output"
OUT_DOCX = OUT / "王威-专业学位研究生学位论文中期考评表-M7最终候选稿.docx"
